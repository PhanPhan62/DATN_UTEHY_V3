...
exports.formatDay = (req, res) => {
  ...
  const NgayBatDau = "2024-03-25",
    MaNhanVien = 3;
  const queryHoadonnhap =
    "insert into hoadonnhap (`SoHoaDonNhap`, `NgayNhap`, `MaNhanVien`, `MaNhaSanXuat`) values( ?, ?, ?, 1);";
  db.query(
    queryHoadonnhap,
    [HDNCode, NgayBatDau, MaNhanVien],
    (err, result) => {
      if (err) {
...
exports.createProduct = (req, res) => {
  try {
    ...
    const query =
      "INSERT INTO sanpham (MaLoai, TenSanPham, MoTaSanPham, MaNSX, MaDonViTinh) VALUES (?, ?, ?, ?, ?)";
    ...
    db.query(query, values, (err, result) => {
      if (err) {
        ...
        const queryHoadonnhap =
          "insert into hoadonnhap (`SoHoaDonNhap`, `NgayNhap`, `MaNhanVien`, `MaNhaSanXuat`) values( ?, ?, ?, 1);";
        const queryCTHoadonnhap =
          "insert into chitiethoadonnhap ( `MaSanPham`, `MaHoaDonNhap`, `SoLuong`, `DonGiaNhap`) values( ?, ?, ?, ?);";
        const now = new Date();
        ...
        db.query(
          ...
          (err, data) => {
            if (err) {
              ...
              res
                .status(200)
                .json({ error: "Đã có lỗi xảy ra ở hoá đơn nhập." });
            } else {
              const idHoaDonNhap = data.insertId;

              db.query(
                queryCTHoadonnhap,
                [sanphamId, idHoaDonNhap, SoLuong, DonGiaNhap],
                (err, datact) => {
                  if (err) {
                    console.error("Lỗi thêm sản phẩm:", err);
                    res.status(200).json({
                      error: "Đã có lỗi xảy ra ở chi tiết hoá đơn nhập."
                    });
                  }
                }
              );
        ...
        const giaSanPhamQuery =
          "INSERT INTO giasanpham (MaSanPham, NgayBatDau, NgayKetThuc, Gia) VALUES (?, ?, ?, ?)";
        const giaSanPhamValues = [
          sanphamId,
          NgayBatDau,
        ...
        db.query(giaSanPhamQuery, giaSanPhamValues, err => {
          if (err) {
            ...
            res
              .status(500)
              .json({ error: "Đã có lỗi xảy ra khi thêm giá sản phẩm." });
          } else {
            // Thông báo chỉ được gửi khi tất cả dữ liệu đã được thêm thành công
            res.status(201).json({
              message: "Sản phẩm và giá sản phẩm đã được thêm thành công."
            });
          }
        });
      }
  ...
  try {
    ...
    db.query(getOldImagesQuery, [id], (err, results) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "Đã có lỗi xảy ra khi cập nhật thông tin sản phẩm." });
      }
      ...
      db.query(deleteImagesQuery, [id], err => {
        if (err) {
          ...
          return res.status(500).json({
            error: "Đã có lỗi xảy ra khi cập nhật thông tin sản phẩm."
          });
        }
        ...
        const updateQuery =
          "UPDATE sanpham SET MaLoai = ?, TenSanPham = ?, MoTaSanPham = ?, MaNSX = ?, MaDonViTinh = ? WHERE id = ?";
        const updateValues = [
          MaLoai,
          TenSanPham,
          MoTaSanPham,
        ...
        db.query(updateQuery, updateValues, err => {
          ...
          const updateGiaQuery =
            "UPDATE giasanpham SET NgayBatDau= null, NgayKetThuc = null, Gia = ? WHERE MaSanPham = ?";
          ...
          db.query(updateGiaQuery, updateGiaValues, err => {
            if (err) {
              ...
              return res
                .status(500)
                .json({ error: "Đã có lỗi xảy ra khi cập nhật giá sản phẩm." });
            }

            const updateCTHDNQuery =
              "update chitiethoadonnhap set MaHoaDonNhap = ?, SoLuong = ?, DonGiaNhap = ? where MaSanPham = ?";
            ...
            db.query(updateCTHDNQuery, updateCTHDNValues, err => {
              if (err) {
                console.error("Lỗi cập nhật giá sản phẩm:", err);
                return res.status(500).json({
                  error: "Đã có lỗi xảy ra khi cập nhật giá sản phẩm."
                });
              }
...
exports.deleteProduct = (req, res) => {
  ...
  db.query(
    "SELECT Anh FROM chitietanh WHERE MaSanPham = ?",
    id,
    (err, results) => {
      if (err) {
