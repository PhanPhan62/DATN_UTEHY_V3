...
exports.cancelOrder = (req, res) => {
  ...
  const query = `CALL cancelDonHang (${id})`;
  db.query(query, (err, results) => {
    if (err) {
      console.error("Lỗi:", err);
      res.status(500).json({ message: "Lỗi", error: err });
...
exports.GiaoHangDonHang = (req, res) => {
  const id = req.params.id;
  const query = `CALL GiaoHangDonHang (${id})`;
  db.query(query, (err, results) => {
    if (err) {
      console.error("Lỗi:", err);
      res.status(500).json({ message: "Lỗi", error: err });
...
exports.getAllOrdersProductDetail = (req, res) => {
  const id = req.params.maChiTietDonHang;
...
exports.createOrder = (req, res) => {
  try {
    ...
    const orderQuery = `INSERT INTO donhang (MaKhachHang, NgayDat, TrangThaiDonHang) VALUES (?, ?, 'Đang chờ duyệt')`;
